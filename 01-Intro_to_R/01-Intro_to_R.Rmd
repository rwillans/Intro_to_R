...
-   I"ll assume no familiarity with R, but will try to take
...
-   The basic logical operators differ as they are symbols not words:
...
-   Printing objects can just be done by calling the object, it doesn"t
...
#Return elements that are "list")
a_vector[a_vector == "list"]
...
Archive Network ("CRAN") and associated mirror sites. Packages on CRAN
have some peer-review and quality checks to ensure they are good
quality.

To install a package:

1.  use the "install.packages" function, passing the package name as a
    string (in quotes). Once installed, it will be available and will
    not need to be installed again unless updated.
2.  To then access the functions or data in the package, load it by
    using the library() function, passing the package name (no quotes).
    "library" needs to be called for each R session

```{r packages, exercise = TRUE, warning= FALSE}

#install the package "fortunes"
install.packages("fortunes", repos = "https://cran.ma.imperial.ac.uk/")

#load the package "fortunes"
library(fortunes)

#run the fortune function for a fortune cookie message relating to packages
fortune(361)
...
-   It"s strongly recommended to work through R exercises using the
...
-   If you are unfamiliar with IDEs, you can think of R as the "car
    ...
    language easier and displays information about what"s happening.
...
-   Whilst a handful of other R IDEs exist, none really comes close to
    RStudio, so it"s worthwhile familiarising yourself with it. It"s not
    like Python where different users may prefer differing IDEs for
    certain purposes.
-   RStudio now offers reasonable Python integration via the reticulate
    package

### Getting started with RStudio

R and RStudio have now been installed on the Python Club server. To
access the NICE RStudio Server interface, you"ll need to be setup with a
username and password via the ops team.
...
characteristics. After loading the tidyverse package suite, we can view
...
for weight retention to avoid creating a 0.98 carat diamond. 2 The
overall relationship of price and weight is exponential - the price jump
...
quiz(question("Question 1: In R, what symbol is used to assign things to objects (the assignment operator)?",
         ...
         allow_retry = TRUE),
     question("Question 2: What index number is the first element in an R list?",
         answer("0", message = "Sorry this is not like Python or C"),
         answer("1", correct = TRUE),
         answer("Trick Question! - you should never reference by position but always by name for more robust code", message = "Lose 10 points for being a smart alec"),
         answer("42", message = "You must be aRthur Dent"),
         random_answer_order = TRUE,
         allow_retry = TRUE)
)
...
quiz(question("Question 3: What is the console command to install the tidyverse package suite?",
...
7.  Print the structure of diamonds by running the code below. How many
    rows and colums does the dataset have? Which columns contain
...
Other specific error messages or questions can often be resolved by a
search on [Google](www.google.co.uk) or [Stack Overflow](https://www.stackoverflow.com). There are also lots of tutorials and short videos on [Youtube](https://www.youtube.com/results?search_query=R+tutorials) if
you know what you"re searching for. There is also a R specific search
engine called [R-Seek](https://www.rseek.org) which narrows down results
to R content only. The [NHS-R community website](https://nhsrcommunity.com) also offers recordings, blogs and
other R resources. Their slack channel is useful for queries and R
related news. [R-bloggers](https://r-bloggers.com) also aggregates R
blogs so can be useful to monitor or search for things of interest. [R weekly](https://rweekly.org) is a mailing list which gives you up to date developments with all things R.
...
[Paul lan der Laken"s](https://paulvanderlaken.com/2017/08/10/r-resources-cheatsheets-tutorials-books/)
continually updated list of R Resources is pretty close to canonical
...
[RStudio"s education pages](https://education.rstudio.com/learn/) have a
...
[R-Charts](https://r-charts.com/) is
a nice repository of charts to demonstrate the possibilities of using R
...
shows you how the BBC make graphics using R, and how to replicate these
...
[Advanced R](https://adv-r.hadley.nz/) by Hadley Wickham. Useful for
those with an existing understanding of programming concepts from other
languages, or advanced R users. **Not** an introductory book.
